...
export class VimWorker {
    ...
    terminate() {
        ...
        console.log("Terminated worker thread. Thank you for working hard!")
    }
    sendStartMessage(msg) {
        this.worker.postMessage(msg);
        console.log("Sent start message", msg)
    }
    ...
    notifyClipboardError() {
        ...
        console.log("Reading clipboard failed. Notify it to worker")
    }
    async responseClipboardText(text) {
        ...
        console.log("Wrote clipboard", encoded.byteLength, "bytes text and notified to worker")
    }
    async requestCmdline(cmdline) {
        ...
        console.log("Result of command", cmdline, ":", msg.success);
        if (!msg.success) {
    ...
    async notifyErrorOutput(message) {
        ...
        console.log("Sent error message output:", message)
    }
    async notifyEvalFuncRet(ret) {
        ...
        console.log("Sent return value of evaluated JS function:", ret)
    }
    async notifyEvalFuncError(msg, err, dontReply) {
        const errmsg = `${msg} for jsevalfunc(): ${err.message}: ${err.stack}`;
        if (dontReply) {
            console.log("Will send error output from jsevalfunc() though the invocation was notify-only:", errmsg);
        ...
        console.log("Sent exception thrown by evaluated JS function:", msg, err)
    }
    onEventDone(doneStatus) {
        ...
        if (this.pendingEvents.length === 0) {
            console.log("No pending event remains after event", done);
            return
        }
        console.log("After", done, "event, still", this.pendingEvents.length, "events are pending");
    ...
    enqueueEvent(status, ...values) {
        ...
        if (this.pendingEvents.length > 1) {
            console.log("Other event is being handled by worker. Pending:", statusName(status), values);
            return
        }
        this.sendEvent(status, values)
    }
    sendEvent(status, values) {
        ...
        console.log("Write event", event, "payload to buffer:", values);
        let idx = 0;
        ...
        console.log("Wrote", idx * 4, "bytes to buffer for event", event);
        Atomics.notify(this.sharedBuffer, 0, 1);
        console.log("Notified event", event, "to worker")
    }
    ...
    recvError(e) {
        console.log("Received an error from worker:", e);
...
export class InputHandler {
    ...
    onKeydown(event) {
        ...
        console.log("onKeydown():", event, event.key, event.keyCode);
        let key = event.key;
        ...
        if (key.length > 1) {
            if (key === "Unidentified" || ctrl && key === "Control" || shift && key === "Shift" || alt && key === "Alt" || meta && key === "Meta") {
                console.log("Ignore key input", key);
                return
            }
        }
...
export class ScreenCanvas {
    ...
    onAnimationFrame() {
        console.log("Rendering", this.queue.length, "events on animation frame");
        this.perfMark("raf");
...
export class VimWasm {
    ...
    start(opts) {
        ...
        console.log("Started with drawer", this.screen)
    }
    async dropFile(name, contents) {
        if (!this.running) {
            throw new Error("Cannot open file since Vim is not running")
        }
        console.log("Handling to open file", name, contents);
        ...
        console.log("Wrote file", name, "to", contents.byteLength, "bytes buffer and notified it to worker")
    }
    ...
    async readFile(reader, file) {
        return new Promise((resolve, reject) => {
            reader.onload = (f => {
                console.log("Read file", file.name, "from D&D:", f);
    ...
    async evalJS(path, contents) {
        console.log("Evaluating JavaScript file", path, "with size", contents.byteLength, "bytes");
        const dec = new TextDecoder;
        const src = '"use strict";' + dec.decode(contents);
        try {
            Function(src)()
        } catch (err) {
            console.log("Failed to evaluate", path, "with error:", err);
            await this.showError(`${err.message}\n\n${err.stack}`)
        }
    }
    async evalFunc(body, args, notifyOnly) {
        console.log("Evaluating JavaScript function:", body, args);
        let f;
        try {
        ...
        if (notifyOnly) {
            console.log("Evaluated JavaScript result was discarded since the message was notify-only:", ret, body);
    ...
    onMessage(msg) {
        ...
        switch (msg.kind) {
            case "draw":
                ...
                console.log("draw event", msg.event);
                break;
            case "done":
            ...
            case "title":
                if (this.onTitleUpdate) {
                    ...
                    console.log("title was updated:", msg.title);
            ...
            case "read-clipboard:request":
                if (this.readClipboard) {
                    ...
                    console.log("Cannot read clipboard because VimWasm.readClipboard is not set");
                    this.worker.notifyClipboardError()
                }
                break;
            case "write-clipboard":
                console.log("Handle writing text", msg.text, "to clipboard with", this.onWriteClipboard);
            ...
            case "export":
                if (this.onFileExport !== undefined) {
                    console.log("Exporting file", msg.path, "with size in bytes", msg.contents.byteLength);
            ...
            case "exit":
                ...
                console.log("Vim exited with status", msg.status);
                this.perf = false;
                this.debug = false;
                ...
                break;
            case "error":
                console.log("Vim threw an error:", msg.message);
